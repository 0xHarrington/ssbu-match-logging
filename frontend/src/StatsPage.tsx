...
const CSS_COLOR = { Shayne: 'var(--shayne)', Matt: 'var(--matt)' } satisfies Record<Player, string>;

/** Player -> dither-kit palette name (see components/dither/palette.ts). */
const CHART_COLOR = { Shayne: 'orange', Matt: 'green' } satisfies Record<Player, 'orange' | 'green'>;

/** Player -> the lowercased key the API uses in `{shayne, matt}` payloads. */
const KEY = { Shayne: 'shayne', Matt: 'matt' } satisfies Record<Player, 'shayne' | 'matt'>;
    ...
    <PageColumn>
      ...
      <div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(280px,1fr))', gap: 20 }}>
          {[
            { name: home, data: charWinRates[hk], color: CSS_COLOR[home] },
            { name: away, data: charWinRates[ak], color: CSS_COLOR[away] },
          ].map(({ name, data, color }) => (
