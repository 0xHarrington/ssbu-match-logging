...
3. ~~**Burn down the anti-slop baseline**~~ — DONE 2026-08-16, same PR that
   installed the linter: all 81 findings across 31 files fixed (mostly
   `satisfies` swaps, `in`-narrowing type guards, and a shared `RowValue`
   contract for the dither engine's rows), and `oxlint` is chained into
   `npm run lint` so CI gates every rule at `error`. Item 2's `no-explicit-any`
